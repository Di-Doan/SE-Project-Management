...
export const addStudentTeam = async (req, res) => {
  ...
  const studentTeamPromise = studentTeamService.getStudentTeam(
    studentId,
    teamId
  );
  const [team, studentTeam] = await Promise.all([
    teamPromise,
    studentTeamPromise,
  ]);
  ...
  if (!student.course?.availablity) {
    return res
      .status(400)
      .json({ message: "Student has joined another team in this course" });
  }
  ...
  try {
    const result = studentTeamService.addStudentTeam(
      studentId,
      teamId,
      connection
    );
    ...
    if (!availability) {
      ...
      return res
        .status(500)
        .json({ message: "Failed to update student availability" });
    }
...
export const removeStudentTeam = async (req, res) => {
  ...
  const studentTeam = await studentTeamService.getStudentTeam(
    studentId,
    teamId
  );
  if (!studentTeam) {
  ...
  try {
    const result = await studentTeamService.removeStudentTeam(
      studentId,
      teamId,
      connection
    );
    ...
    return res
      .status(200)
      .json({ message: "Successfully removed student team" });
  } catch (err) {
...
export const getTeamIdOfStudent = async (req, res) => {
  const { id: courseId } = req.params;
  const { id } = req.user;

  try {
    const teamId = await teamService.getTeamIDByCourseAndStudent(courseId, id);
	return res.status(200).json(teamId);
  } catch (error) {
	return res.status(404).json({ message: "Failed to find team Id" }); 
  }
};
