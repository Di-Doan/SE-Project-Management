...
export const createChat = async (description, connection) => {
	...
	try {
		const [results] = await db.query('INSERT INTO Chat VALUES (?, NOW())', [
...
export const getMessageLog = async (chat_id, filter) => {
	...
	const queryString = `
        SELECT * FROM Message AS message
...
export const postMessage = async (message, chat_id, student_id) => {
	try {
		...
		// check student id
		[results] = await pool.query('SELECT * FROM Student WHERE Student.student_id = ?', [
			student_id,
		]);
		if (results.length === 0) return null;
