	...
	return (
		<div className='sidebar'>
			...
			<div className='BuildingITSystem'>
				<div className='course' onClick={() => onSelectGroup('Tutorial 1')}>
					{user && user.courses && user.courses[0] ? user.courses[0].name : 'empty'}
				</div>
				<div className='sub-groups'>
					<div className='sub-group' onClick={() => onSelectGroup('Tutorial 1')}>
						Tutorial 1
					</div>
					<div className='sub-group' onClick={() => onSelectGroup('Tutorial 2')}>
						Tutorial 2
					</div>
				</div>
			</div>
			...
			<div className='user-details'>
				...
				<div className='user-name'>{user.fullname ? user.fullname : 'Full Name'}</div>
			</div>
		</div>
	);
};
